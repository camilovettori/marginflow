"use client"

import type { ReactNode } from "react"

type WorkspacePageHeaderProps = {
  label: string
  title: ReactNode
  subtitle?: ReactNode
  companyName?: ReactNode
  companyMeta?: ReactNode
  companyBadge?: ReactNode
  actions?: ReactNode
}

export default function WorkspacePageHeader({
  label,
  title,
  subtitle,
  actions,
}: WorkspacePageHeaderProps) {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white px-4 py-4 shadow-none md:px-5 md:py-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 space-y-2">
          <div className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">
            <span>{label}</span>
          </div>

          <div className="max-w-4xl">
            <h1 className="text-3xl font-semibold tracking-tight text-zinc-950 md:text-[2.55rem]">
              {title}
            </h1>
            {subtitle ? (
              <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-500">{subtitle}</p>
            ) : null}
          </div>
        </div>

        <div className="flex shrink-0 flex-col items-start gap-2 lg:items-end">
          {actions ? <div className="flex flex-wrap items-center gap-2">{actions}</div> : null}
        </div>
      </div>
    </div>
  )
}
